...
Enable the `reader` feature to get tier-data parsers (`tier0::Tier0Data`, `tier1::Tier1Row`) and Fp serialization helpers (`fp_utils`). The default `zakura` backend keeps this crate lightweight until `reader` activates the crypto dependencies. An upstream reader must use `default-features = false, features = ["reader", "upstream"]`. The two backend features are mutually exclusive.

...
fn encode_tree(tree: &PirTree) -> Result<Vec<u8>> {
    ...
    for (dst, &src) in empty_hashes.iter_mut().zip(tree.empty_hashes.iter()) {
        *dst = fp_to_bytes(src);
    }
    let wire = PirTreeWire {
        pir_root: fp_to_bytes(tree.pir_root),
        circuits_root: fp_to_bytes(tree.circuits_root),
        levels,
        ranges,
        empty_hashes,
    };
...
fn decode_tree(bytes: &[u8]) -> Result<PirTree> {
    ...
    let circuits_root = fp_from_bytes(wire.circuits_root)?;
    ...
    for (dst, &src) in empty_hashes.iter_mut().zip(wire.empty_hashes.iter()) {
        *dst = fp_from_bytes(src)?;
    }
    Ok(PirTree {
        pir_root,
        circuits_root,
...
mod tests {
    ...
    fn tiny_tree() -> PirTree {
        ...
        let circuits_root = crate::extend_root(pir_root, &empty_hashes);
        PirTree {
            pir_root,
            circuits_root,
            levels,

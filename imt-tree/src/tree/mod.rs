...
/// Pre-compute the empty subtree hash at each tree level (K=1 gap-range leaves).
///
...
/// Pre-compute the empty subtree hash at each tree level (K=2 punctured-range leaves).
///
/// `empty[0] = hash3(0, 0, 0)` -- the commitment of an all-zero punctured range.
/// `empty[i] = hash(empty[i-1], empty[i-1])` for higher levels.
///
/// This MUST be used instead of [`precompute_empty_hashes`] when building or
/// verifying punctured-range trees so that padding leaves are consistent with
/// the `Poseidon3` leaf commitment scheme.
pub fn precompute_empty_hashes_k2() -> [Fp; TREE_DEPTH] {
    let hasher = PoseidonHasher::new();
    let mut empty = [Fp::default(); TREE_DEPTH];

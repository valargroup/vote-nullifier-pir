...
- `voting-config.json` cannot be fetched or decoded, or `snapshot_height` is missing, while bootstrap is still enabled (default: non-empty voting-config URL). For offline-only disks, set `SVOTE_PIR_VOTING_CONFIG_URL=` (or legacy `SVOTE_VOTING_CONFIG_URL=`) so bootstrap is skipped and pre-staged files under `SVOTE_PIR_DATA_DIR` are served.

Resolution hints:

- Production: defaults are usually correct; override `SVOTE_PIR_VOTING_CONFIG_URL` (or legacy `SVOTE_VOTING_CONFIG_URL`) only for a mirror or staging config. For fully local tiers, set it to empty to disable bootstrap.
- Confirm `SVOTE_PIR_PRECOMPUTED_BASE_URL` (or legacy `SVOTE_PRECOMPUTED_BASE_URL`) when relying on CDN tier download (default points at production object storage).
...
The server can emit errors and traces to Sentry. Create a project at [sentry.io](https://sentry.io), copy the DSN, and set `SENTRY_DSN`. The in-process snapshot watchdog emits stale-snapshot events through Sentry when `SVOTE_PIR_STALE_THRESHOLD_SECS` (legacy `SVOTE_STALE_THRESHOLD_SECS`) is non-zero and a DSN is present; tune `SVOTE_PIR_WATCHDOG_TICK_SECS` (legacy `SVOTE_WATCHDOG_TICK_SECS`) for how often it checks gauges versus the threshold.
...
| `SVOTE_PIR_VOTING_CONFIG_URL` (legacy `SVOTE_VOTING_CONFIG_URL`) | Empty string skips voting-config fetch and height cap; non-empty requires `snapshot_height` |
...
The `nf-server serve` CLI (see `nf-server serve --help`) reads environment variables including: `SVOTE_PIR_PORT`, `SVOTE_PIR_DATA_DIR` (legacy `SVOTE_DATA_DIR`), `SVOTE_PIR_VOTING_CONFIG_URL` (legacy `SVOTE_VOTING_CONFIG_URL`; defaults to the production voting-config URL when neither is set), `SVOTE_PIR_PRECOMPUTED_BASE_URL` (legacy `SVOTE_PRECOMPUTED_BASE_URL`), `SVOTE_PIR_MAINNET_RPC_URL` (legacy `SVOTE_MAINNET_RPC_DIR`), `LWD_URLS` (comma-separated override for lightwalletd), `SVOTE_PIR_BOOTSTRAP_TIMEOUT_SECS` (legacy `SVOTE_BOOTSTRAP_TIMEOUT_SECS`), `SVOTE_PIR_STALE_THRESHOLD_SECS` (legacy `SVOTE_STALE_THRESHOLD_SECS`), `SVOTE_PIR_WATCHDOG_TICK_SECS` (legacy `SVOTE_WATCHDOG_TICK_SECS`), `SVOTE_PIR_VOTE_CHAIN_URL` (legacy `SVOTE_VOTE_CHAIN_URL`), and `SENTRY_DSN`.
...
| Topic | Decision |
|-------|----------|
| Voting-config unavailable when its URL is set | With the default non-empty URL (or any non-empty override), fetch and `snapshot_height` are required or startup fails. **Offline / manual disks:** explicitly clear `SVOTE_PIR_VOTING_CONFIG_URL` (or legacy `SVOTE_VOTING_CONFIG_URL`) and stage tier files under `SVOTE_PIR_DATA_DIR` yourself. |
...
- Document `SVOTE_PIR_VOTE_CHAIN_URL` (optional; active-round guard for `POST /snapshot/prepare`; legacy `SVOTE_VOTE_CHAIN_URL`) in operator-facing docs when stable.

...
- Add explicit depth and tier-split metadata to `/root`, and require PIR client
  ...
  any private query.
- Rename the Rust fields `root25` and `root29` to `pir_root` and
  `circuit_root`, while retaining `root25` and `root29` on the wire for rolling
  compatibility; descriptive wire names are also accepted when reading.

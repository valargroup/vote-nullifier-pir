...
impl Config {
    ...
    /// Current production bucket origin used for precomputed snapshots.
...
/// enabled (URL non-empty, including the baked-in default) but the config
/// cannot be read, no target height can be resolved, or for I/O errors while
...
mod tests {
    ...
    #[test]
    fn default_precomputed_base_url_uses_current_origin() {
        assert_eq!(
